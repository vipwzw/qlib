...
class OptimizedNewsSentimentStrategy:
    """优化的新闻情感因子策略 - 平衡信号匹配与盈利能力"""
    ...
    def __init__(self, config_path: str = "configs/config.yaml"):
        ...
        # 优化后的策略参数
        self.buy_threshold = 0.05    # 买入阈值（保持不变）
        self.sell_threshold = -0.07  # 卖出阈值（降低敏感性）
        self.max_holding_days = 12   # 最大持仓天数（延长持仓）
        self.stop_loss = -0.04       # 止损阈值（稍微收紧）
        self.take_profit = 0.04      # 止盈阈值（降低以增加触发）
    ...
    def run_optimized_backtest(self, start_date: str = '2023-01-01', 
                             end_date: str = '2024-01-01', 
                             initial_capital: float = 100000) -> pd.DataFrame:
        """运行优化的回测"""
        self.logger.info("🚀 开始运行优化的新闻情感策略回测...")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="优化的新闻情感因子策略")
    ...
    try:
        # 初始化策略
        strategy = OptimizedNewsSentimentStrategy(args.config)
        
        # 运行回测
        results, trade_log = strategy.run_optimized_backtest(
            args.start_date, 
            args.end_date, 
        ...
        if args.save_results:
            ...
            timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
            results_file = results_dir / f"optimized_backtest_results_{timestamp}.csv"
